;(function () {
	...
	function getPreview(query, content, previewLength) {
		...
		if (match >= 0) {
			...
			preview = preview.replace(new RegExp("(" + parts.join("|") + ")", "gi"), "<strong class='search-results-highlight'>$1</strong>");
		} else {
					...
					"<li>" +
						"<a href='" + item.url.trim() + "'>" +
							"<div class='notification is-dark'>" +
								"<h4 class='title is-5'>" + titlePreview + "</h4>" +
							"</div>" +
						"</a>" +
					"</li>";
			});

			searchResultsStatusEl.innerHTML = results.length + " resultado" + (results.length === 1 ? "" : "s") + " para <strong>" + escapeHtml(query) + "</strong>";
			searchResultsEl.innerHTML = resultsHTML;
		} else {
			searchResultsStatusEl.innerHTML = "Nada encontrado" + (query ? " para <strong>" + escapeHtml(query) + "</strong>" : "");
	...
	function runSearch(query) {
		...
		searchResultsStatusEl.innerHTML = "Carregando resultados para <strong>" + escapeHtml(query) + "</strong>...";
	...
	var query = decodeURIComponent((getQueryVariable("s") || "").replace(/\+/g, "%20")),
